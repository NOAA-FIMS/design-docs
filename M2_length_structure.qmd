...
| Alaska method           | allow length-based selectivity                                               | does not allow for growth estimation or fishery impact on length structure                       |
| SAM method              | State-space flexibility                                                      | Current practices in US focused on parametric modeling. Length not incorporated in current form. |
| Age-length key (SS3)    | allow for selectivity at length and tracks numbers- and age-at-length        | can be slow due to dense matrices. does not track length specific fishing impacts.               |
| Full age-length         | does not assume fixed age-length key. Potential to scale complexity/runtime. | could be computationally intensive based on bin size.                                            |
